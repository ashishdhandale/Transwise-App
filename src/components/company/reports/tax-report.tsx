...
const calculateGstAmount = (booking: Booking): number => {
    if (!booking.isGstApplicable) return 0;
    ...
    // Assuming gst percentage is stored somewhere or can be derived.
    // For now, let's derive it from the total amount.
...
export function TaxReport() {
    ...
    const taxBookings = useMemo(() => {
        return allBookings
            .filter(b => b.taxPaidBy === 'Transporter')
            .map(b => ({
                ...b,
                                ...
                                {taxBookings.length > 0 ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={9} className="text-center h-24 text-muted-foreground">
                                            No bookings found where tax is paid by the transporter.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
