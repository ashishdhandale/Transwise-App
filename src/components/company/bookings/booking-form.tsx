...
const generateLrNumber = (bookings: Booking[], prefix?: string) => {
    const autoGeneratedBookings = bookings.filter(b => b.source === 'System');

    if (autoGeneratedBookings.length === 0) {
        return prefix ? `${prefix}01` : '1';
    }

    const numericSequences = autoGeneratedBookings.map(b => {
        const lr = b.lrNo;
        if (prefix && lr.startsWith(prefix)) {
            const numPart = lr.substring(prefix.length);
            return parseInt(numPart, 10);
        }
        if (!prefix && /^\d+$/.test(lr)) {
            return parseInt(lr, 10);
        }
        return NaN; // Not a valid sequence member
    }).filter(num => !isNaN(num));

    const maxNum = numericSequences.length > 0 ? Math.max(...numericSequences) : 0;
